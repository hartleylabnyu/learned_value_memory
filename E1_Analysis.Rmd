...
pa_data <- memData %>% 
  group_by(sub, block, paTrial, associateSide) %>% 
  summarize(paResp = mean(paResp, na.rm = TRUE)) %>% 
  drop_na(.)

#Compute PA accuracy for each trial for each subject
pa_data <- pa_data %>% 
  mutate(acc = case_when(paResp - associateSide == 0 ~ 1, 
                         paResp - associateSide != 0 ~ 0))
...
paAccData.sub <- paAccData.subBlocks  %>% 
  ...
  mutate(include = case_when(include1 + include2 == 2 ~ 1, 
                             include1 + include2 < 2 ~0)) %>%
  select(-c(include1, include2))
...
## Age statistics
```{r age histogram}
ageHist <- ggplot(subInfo, aes(x = trunc(age), fill = as.factor(gender))) +
  geom_histogram(color = "white", bins = 14) +
  scale_fill_manual(values = c(color1, color3), name = "Sex") +
  xlab("Age") +
  kate.theme
ageHist
```
...
## Preregistered frequency task analyses

```{r compute frequency task accuracy}
#compute accuracy for each trial
freqData %<>% mutate(freqAcc = case_when(freqResp == 0 & appearanceCount == 1 ~ 1, 
                                         freqResp == 0 & appearanceCount > 1 ~ 0, 
                                         freqResp == 1 & appearanceCount > 1 ~ 1, 
                                         freqResp == 1 & appearanceCount == 1 ~ 0))

freqData$ageScaled <- scale_this(freqData$age)
freqData$IQScaled <- scale_this(freqData$IQ)
freqData$appearanceCountScaled <- scale_this(freqData$appearanceCount)
```

### Frequency task accuracy
```{r prereg freq acc maximal model, eval = F}
freqAcc.prereg.maximal <- mixed(freqAcc ~ ageScaled * block * appearanceCountScaled * IQScaled + (block* appearanceCountScaled |sub) + (ageScaled * block * appearanceCountScaled * IQScaled|item), data = freqData, family = binomial, method = "LRT", control = glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)))

# does not converge
```

```{r prereg freq acc model 2, eval = F}
#remove correlations in REs
freqAcc.prereg.2 <- mixed(freqAcc ~ ageScaled * block * appearanceCountScaled * IQScaled + (block * appearanceCountScaled||sub) + (ageScaled * block * appearanceCountScaled * IQScaled||item), data = freqData, family = binomial, method = "LRT", control = glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)))

#does not converge
```

```{r prereg freq acc model 3, eval = F}
#remove interactions in REs
freqAcc.prereg.3 <- mixed(freqAcc ~ ageScaled * block * appearanceCountScaled * IQScaled + (block + appearanceCountScaled||sub) + (ageScaled + block + appearanceCountScaled + IQScaled||item), data = freqData, family = binomial, method = "LRT", control = glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)))

#does not converge
```

```{r prereg freq acc model 4, eval = F}
#remove age and IQ from item RE
freqAcc.prereg.4 <- mixed(freqAcc ~ ageScaled * block * appearanceCountScaled * IQScaled + (block + appearanceCountScaled||sub) + (appearanceCountScaled ||item), data = freqData, family = binomial, method = "LRT", control = glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)))

#does not converge
```

```{r prereg freq acc model 5}
#remove block from subject RE
freqAcc.prereg.5 <- mixed(freqAcc ~ ageScaled * block * appearanceCountScaled * IQScaled + (appearanceCountScaled||sub) + (appearanceCountScaled||item), data = freqData, family = binomial, method = "LRT", control = glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)))

#display
nice(freqAcc.prereg.5)

#run glmer model
freqAcc.prereg.5.glmer <- mixed(freqAcc ~ ageScaled * block * appearanceCountScaled * IQScaled + (appearanceCountScaled||sub) + (appearanceCountScaled||item), data = freqData, family = binomial, method = "LRT", control = glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)), return = "merMod")
```


### Frequency task reaction times
```{r frequency RT data processing}
freqRTUnfiltered <- freqData %>%
  filter(freqAcc == 1)

freqRTUnfiltered$ageScaled <- scale_this(freqRTUnfiltered$age)
freqRTUnfiltered$IQScaled <- scale_this(freqRTUnfiltered$IQ)
freqRTUnfiltered$appearanceCountScaled <- scale_this(freqRTUnfiltered$appearanceCount)
```

```{r frequency RT prereg maximal model, eval = F}
freqRT.prereg.maximal <- mixed(freqRT ~ ageScaled * appearanceCountScaled * block * IQScaled + (appearanceCountScaled * block|sub) + (ageScaled * appearanceCountScaled * block * IQScaled|item), data = freqRTUnfiltered, method = "KR", control = lmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)))
#does not converge
```

```{r frequency RT prereg model 2}
#remove correlations from random slopes
freqRT.prereg.2 <- mixed(freqRT ~ ageScaled * appearanceCountScaled * block * IQScaled + (appearanceCountScaled * block||sub) + (ageScaled * appearanceCountScaled * block * IQScaled||item), data = freqRTUnfiltered, method = "KR", control = lmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)), expand_re = T)

nice(freqRT.prereg.2)

#get lmer model
freqRT.prereg.2.lmer <- mixed(freqRT ~ ageScaled * appearanceCountScaled * block * IQScaled + (appearanceCountScaled * block||sub) + (ageScaled * appearanceCountScaled * block * IQScaled||item), data = freqRTUnfiltered, method = "KR", control = lmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)), expand_re = T, return = "merMod")

```

## Non-preregistered, but more interpretable frequency task models

_First, we will ensure that all subjects responded to >10% of repeated trials, indicating that they understood the task instructions._

```{r frequency task data processing}
...
explicitFreqReports_badSubs <- temp[which(temp$badFreqSub == 1),] %>% 
  group_by(sub, block, freqCond) %>% 
  summarise(meanFreqReport = mean(memFreqResp, na.rm = TRUE))

explicitFreqReports <- temp %>% 
  group_by(badFreqSub, freqCond) %>% 
  summarise(meanFreqReport = mean(memFreqResp, na.rm = TRUE))
...
#display
nice(freqFirstAppearanceAcc.maximal)
```
...
freqAccPlotData <- freqDataRepeated %>% 
  mutate(IQGroup = ntile(IQ, 2)) %>% 
  group_by(block, ageGroup, IQGroup) %>% 
  summarise( meanFreqAcc = mean(freqAcc, na.rm = T),
             sdFreqAcc = sd(freqAcc, na.rm = T),
             N = n(),
             seAcc = sdFreqAcc / sqrt(N))
...
freqRTPlotData <- freqRTdata %>% 
  group_by(ageGroup, appearanceCount) %>% 
  summarise(meanFreqRT = mean(freqRT, na.rm = T), 
            sdFreqRT = sd(freqRT, na.rm = T), N = n(), 
            seRT = sdFreqRT/sqrt(N))
...
#drop unused factors
memFreqData <- droplevels(memFreqData)
```

## Preregistered analysis of explicit frequency reports
_Here we will run mixed-effects models to determine the effects of age, IQ, block, and frequency condition on participants' explicit reports of item frequency._

```{r prereg explicit freq model, eval = F}
freqResponse.maximal <- mixed(memFreqResp ~ freqCondFactor * block * ageScaled * IQScaled + (freqCondFactor*block|sub) + (freqCondFactor*block*ageScaled*IQScaled|item), data = memFreqData, method = "LRT", control = lmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)))

#does not converge
```

```{r prereg explicit freq model 2}
#remove correlations in REs
freqResponse.2 <- mixed(memFreqResp ~ freqCondFactor * block * ageScaled * IQScaled + (freqCondFactor*block||sub) + (freqCondFactor*block*ageScaled*IQScaled||item), data = memFreqData, method = "KR", control = lmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)), expand_re = T)

nice(freqResponse.2)

#return lmer model
freqResponse.2.lmer <- mixed(memFreqResp ~ freqCondFactor * block * ageScaled * IQScaled + (freqCondFactor*block||sub) + (freqCondFactor*block*ageScaled*IQScaled||item), data = memFreqData, method = "KR", control = lmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)), expand_re = T, return = "merMod")
```


### Plot: Average frequency responses (individual subjects)
```{r freq response error plot individual subs}
meanFreqResponsePlotSubData <- memFreqData  %>% 
  group_by(age, sub, freqCondFactor) %>% 
  summarise(meanResp = mean(memFreqResp, na.rm = T))

#create order factor
plotOrder <- meanFreqResponsePlotSubData  %>% 
  group_by(age, sub) %>% 
  summarise(minResp = min(meanResp, na.rm = T)) %>%
  ungroup () %>%
  arrange(minResp) %>%
  mutate(order = order(minResp))

#merge
meanFreqRespPlotData <- merge(meanFreqResponsePlotSubData, plotOrder, by = c("age", "sub"), all = T)

#plot
meanFreqRespSubPlot <- ggplot(meanFreqRespPlotData, aes(x = order, color = freqCondFactor, y = meanResp, group = sub)) +
  geom_point(size = 3, group = sub) +
  geom_line(color = color2) +
  xlab("") +
  ylab("Mean Frequency Report") +
...
#ggsave('meanFreqResps.png', meanFreqRespSubPlot, dpi = 1000, width = 8, height = 4, units = "in")
```
...
### Plot: Mean frequency reports by age group, block, and frequency condition
```{r plot effects of block and frequency condition on frequency response}

memFreqPlotData <- memFreqData %>%
  group_by(freqCondFactor, ageGroup, block) %>%
  summarize(meanFreqResp = mean(memFreqResp),
            N = n(), 
            sdFreqResp = sd(memFreqResp),
            seFreqResp = sdFreqResp/sqrt(N))

respLimits <- aes(ymax = meanFreqResp + seFreqResp, ymin = meanFreqResp -seFreqResp)

freqRespPlot <- ggplot(memFreqPlotData, aes(x = freqCondFactor, fill = block, y = meanFreqResp)) + 
  geom_bar(stat = "identity", position = "dodge") +
  geom_errorbar(respLimits, width = .1, position = position_dodge(width = .9)) +
  facet_wrap(~ageGroup) +
  scale_fill_manual(values = c(color1, color3), name = "Block") +
  ylab("Mean Frequency Response") +
...
## Non-preregistered but more interpretable frequency report analyses
...
freqResponseErrorPlotData <- memFreqData %>% 
  mutate(IQGroup = ntile(IQ, 2)) %>% 
  group_by(ageGroup, IQGroup) %>% 
  summarise(meanError = mean(memFreqRespErrorMagnitude, na.rm = T), sdError = sd(memFreqRespErrorMagnitude, na.rm = T), N = n(), seError = sdError/sqrt(N))
...
### Plot: Frequency response error magnitudes (individual subjects)
```{r freq response error mag plot individual subs}
freqResponseErrorPlotSubData <- memFreqData  %>% 
  group_by(age, sub, freqCondFactor) %>% 
  summarise(meanError = mean(memFreqRespErrorMagnitude, na.rm = T))

freqRespErrorSubPlot <- ggplot(freqResponseErrorPlotSubData, aes(x = age, color = freqCondFactor, y = meanError)) +
  geom_point(alpha = .7, size = 3) +
  #geom_smooth(method = "lm", aes(fill = freqCondFactor)) +
  xlab("Age") +
...
#Remove correlations between random slopes and intercepts
memAcc.2 <- mixed(memAcc ~ ageScaled * freqCondFactor * block * IQScaled + (freqCondFactor*block||sub) + (freqCondFactor*block*ageScaled*IQScaled||item), data = memData, family = "binomial", method = "LRT", control = glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)), expand_re = T)

#does not converge
```

```{r memory accuracy model 3}
# Remove interactions from random slopes
memAcc.3 <- mixed(memAcc ~ ageScaled * freqCondFactor * block * IQScaled + (freqCondFactor+block||sub) + (freqCondFactor+block+ageScaled+IQScaled||item), data = memData, family = "binomial", method = "LRT", control = glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)), expand_re = T)

nice(memAcc.3)
```


```{r memory accuracy maximal model adults only}
#create data frame just with adult data
memData.adults <- memData %>% filter(ageGroup == "Adults")

# Run memory accuracy model with just adult data
memAcc.adults <- mixed(memAcc ~ freqCondFactor * block + (freqCondFactor*block||sub) + (freqCondFactor*block||item), data = memData.adults, family = "binomial", method = "LRT", control = glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)), expand_re = T)

memAcc.adults
```

## Plot: Effects of frequency condition, block, and age on memory
```{r memory accuracy plot, fig.width = 10, fig.height = 5}
memPlotData <- memData %>% 
  mutate(IQGroup = ntile(IQ, 2)) %>% 
  group_by(sub, ageGroup, freqCondFactor, block) %>% 
...
memPlotData <- memData %>% 
  mutate(IQGroup = ntile(IQ, 2)) %>% 
  group_by(ageGroup, freqCondFactor, IQGroup) %>% 
  summarise(meanAcc = mean(memAcc, na.rm = T), sdAcc = sd(memAcc, na.rm = T), N = n(), seAcc = sdAcc/sqrt(N))
...
### Exploratory analysis: How many participants performed below chance-level on the memory test?

#### Threshold = 1 item correct
```{r threshold 1 memory stats}
chanceLevelMemory <- memData %>%
  group_by(sub, age, ageGroup, block) %>%
  summarize(meanAcc = mean(memAcc, na.rm = T), N = n()) %>%
  arrange(meanAcc) %>%
  mutate(belowChance = case_when(meanAcc < (1/16) ~ 'Below Chance', meanAcc > (0/16) ~ 'Above Chance'))

belowChanceMemoryStats <- chanceLevelMemory %>%
  group_by(ageGroup, belowChance, block) %>%
  summarize(N = n(), meanAge = mean(age), sdAge = sd(age))
pander(belowChanceMemoryStats)

#filter memory data
memDataFiltered <- merge(memData, chanceLevelMemory, by = c("ageGroup", "sub", "age", "block")) %>%
  filter(belowChance == 'Above Chance') %>%
  droplevels()

memDataFiltered$ageScaled <- scale_this(memDataFiltered$age)
memDataFiltered$IQScaled <- scale_this(memDataFiltered$IQ)
```

```{r threshold 1 memory model}
#start with same memory model run on full data set
memAcc.threshold1.1 <- mixed(memAcc ~ ageScaled * freqCondFactor * block * IQScaled + (freqCondFactor + block||sub) + (ageScaled + freqCondFactor + block + IQScaled||item), data = memDataFiltered, family = "binomial", method = "LRT", control = glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)), expand_re = T)

nice(memAcc.threshold1.1)

memAcc.threshold1.glmer <- mixed(memAcc ~ ageScaled * freqCondFactor * block * IQScaled + (freqCondFactor + block||sub) + (ageScaled + freqCondFactor + block + IQScaled||item), data = memDataFiltered, family = "binomial", method = "LRT", control = glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)), expand_re = T, return = "merMod")

```

#### Plot: Memory accuracy (below 1 item correct removed)
```{r memory accuracy plot threshold 1}
memPlotData <- memDataFiltered %>% 
  group_by(sub, ageGroup, freqCondFactor) %>% 
  summarise(meanAcc = mean(memAcc, na.rm = T), sdAcc = sd(memAcc, na.rm = T), N = n(), seAcc = sdAcc/sqrt(N))

#compute error bar limits
errorBars <- aes(ymax = meanAcc + seAcc, ymin = meanAcc - seAcc)

memAccFilteredPlot <- ggplot(memPlotData, aes(x = freqCondFactor, fill = freqCondFactor, y = meanAcc)) +
  geom_boxplot(position = position_dodge(.8), alpha = .8) +
  #geom_errorbar(errorBars, position = position_dodge(width = .9), width = .1) +
  facet_wrap(~ageGroup) +
  coord_cartesian(ylim = c(0, 1)) +
...
#### Threshold = 2 items correct
```{r threshold 2 memory stats}
chanceLevelMemory <- memData %>%
  group_by(sub, age, ageGroup, block) %>%
  summarize(meanAcc = mean(memAcc, na.rm = T), N = n()) %>%
  arrange(meanAcc) %>%
  mutate(belowChance = case_when(meanAcc < (2/16) ~ 'Below Chance', meanAcc > (1/16) ~ 'Above Chance'))
...
```{r threshold 2 memory model}
#start with same memory model run on full data set
memAcc.threshold2.1 <- mixed(memAcc ~ ageScaled * freqCondFactor * block * IQScaled + (freqCondFactor + block||sub) + (ageScaled + freqCondFactor + block + IQScaled||item), data = memDataFiltered, family = "binomial", method = "LRT", control = glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)), expand_re = T)

nice(memAcc.threshold2.1) 

memAcc.threshold2.glmer <- mixed(memAcc ~ ageScaled * freqCondFactor * block * IQScaled + (freqCondFactor + block||sub) + (ageScaled + freqCondFactor + block + IQScaled||item), data = memDataFiltered, family = "binomial", method = "LRT", control = glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)), expand_re = T, return = "merMod")
```

#### Plot: Memory accuracy (below 2 items correct removed)
```{r memory accuracy plot threshold 2}
memPlotData <- memDataFiltered %>% 
  group_by(sub, ageGroup, freqCondFactor) %>% 
  summarise(meanAcc = mean(memAcc, na.rm = T), sdAcc = sd(memAcc, na.rm = T), N = n(), seAcc = sdAcc/sqrt(N))
...
#### Threshold = 3 items correct
```{r threshold 3 memory stats}
chanceLevelMemory <- memData %>%
  group_by(sub, age, ageGroup, block) %>%
  summarize(meanAcc = mean(memAcc, na.rm = T), N = n()) %>%
  arrange(meanAcc) %>%
  mutate(belowChance = case_when(meanAcc < (3/16) ~ 'Below Chance', meanAcc > (2/16) ~ 'Above Chance'))
...
```{r threshold 3 memory model}
#start with same memory model run on full data set
memAcc.threshold3.1 <- mixed(memAcc ~ ageScaled * freqCondFactor * block * IQScaled + (freqCondFactor + block||sub) + (ageScaled + freqCondFactor + block + IQScaled||item), data = memDataFiltered, family = "binomial", method = "LRT", control = glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)), expand_re = T)

nice(memAcc.threshold3.1) 

memAcc.threshold3.glmer <- mixed(memAcc ~ ageScaled * freqCondFactor * block * IQScaled + (freqCondFactor + block||sub) + (ageScaled + freqCondFactor + block + IQScaled||item), data = memDataFiltered, family = "binomial", method = "LRT", control = glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)), expand_re = T, return = "merMod")
```

#### Plot: Memory accuracy (below 3 items correct removed)
```{r memory accuracy plot threshold 3}
memPlotData <- memDataFiltered %>% 
  group_by(sub, ageGroup, freqCondFactor) %>% 
  summarise(meanAcc = mean(memAcc, na.rm = T), sdAcc = sd(memAcc, na.rm = T), N = n(), seAcc = sdAcc/sqrt(N))
...
#### Plot: Memory accuracy IQ effects (below 3 items correct removed)
```{r memory accuracy plot threshold 3 IQ interaction}
memAccIQPlot <- plot_model(memAcc.threshold3.glmer, type = "pred", terms = c("IQScaled", "freqCondFactor", "ageScaled")) +
  kate.theme
memAccIQPlot
```

# Relation between learning environmental statistics and memory encoding
_As specified in our preregistration, we will now run a regression examining the effects of frequency distance on memory benefit score, to determine if participants who showed the most evidence of learning the item frequencies also demonstrated the greatest effect of frequency condition on encoding._
...
temp <- memData %>% 
  group_by(sub, freqCond, block, age, IQ, ageGroup) %>% 
...
temp2 <- memData %>% 
  group_by(sub, block, age, ageGroup) %>% 
  summarise(overallMeanMem = mean(memAcc, na.rm = TRUE))

  #merge data frames and compute memBenefitIndex
memBenefit <- merge(temp, temp2, by = c("sub", "block", "age", "ageGroup"), all = TRUE) %>% 
...
meanZs <- memFreqData %>% 
  group_by(sub, block, freqCond, age, IQ, ageGroup) %>% 
...
## Effect of age on frequency distance
```{r frequency index by age model}
freqIndexAge.model <- mixed(freqDist ~ ageScaled * IQScaled + (1|sub), data = subIndices, method = "KR", control = lmerControl(optCtrl=list(maxfun=1e6)))

nice(freqIndexAge.model)
```

## Plot: Relation between age and frequency distance
```{r frequency distance age plot}
freqDistAgePlot <- ggplot(subIndices, aes(y = freqDist, x = age), color = color3, fill = color3) +
  geom_point(color = color3) +
  geom_smooth(method = "lm", color = color3, fill = color3) +
  #facet_wrap(~IQGroup) +
  xlab("Age") +
  ylab("Frequency Distance") +
  kate.theme
freqDistAgePlot
```

## Relation between frequency distance and frequency report error magnitude
```{r freq dist error magnitude data processing}
#For each subject, need to compute mean frequency error magnitude
subFreqErrorMag <- memFreqData %>%
  group_by(sub, block) %>%
  summarize(meanFreqErrorMag = mean(memFreqRespErrorMagnitude, na.rm = T))

#Merge with sub indices
subIndices <- merge(subIndices, subFreqErrorMag, by = c("sub", "block"), all = T)
```

```{r freq dist error magnitude model}
subIndices$meanFreqErrorMagScaled <- scale_this(subIndices$meanFreqErrorMag)

freqDistErrorMag.model <- mixed(freqDist ~ meanFreqErrorMagScaled * IQScaled *ageScaled + (1|sub), data = subIndices, method = "KR", control = lmerControl(optCtrl=list(maxfun=1e6)))

nice(freqDistErrorMag.model)
```

## Plot: Frequency error, frequency distance, age, IQ
```{r freq dist error magnitude model plot}
freqDistErrorMag.model.lmer <- mixed(freqDist ~ meanFreqErrorMag * IQScaled *ageScaled + (1|sub), data = subIndices, method = "KR", control = lmerControl(optCtrl=list(maxfun=1e6)), return = "merMod")

#summary(freqDistErrorMag.model.lmer)

plot_model(freqDistErrorMag.model.lmer, type = "int", terms = c("meanFreqErrorMag", "age", "IQ"))
```


## Plot: Frequency error and frequency distance
```{r freq dist error magnitude plot}
freqDistErrorMagPlot <- ggplot(subIndices, aes(y = freqDist, x = meanFreqErrorMag)) +
  geom_point(color = color3) +
  geom_smooth(method = "lm", color = color3, fill = color3) +
  xlab("Frequency Response Error Magnitude") +
  ylab("Frequency Distance") +
  kate.theme
freqDistErrorMagPlot
```

## Relation between memory benefit, frequency error, and age
```{r mem benefit error magnitude model}
memBenefit.errorMag.model <- mixed(memBenefitIndex ~ meanFreqErrorMag * IQScaled *ageScaled + (1|sub), data = subIndices, method = "KR", control = lmerControl(optCtrl=list(maxfun=1e6)))

nice(memBenefit.errorMag.model)
```

```{r mem benefit error magnitude plot}
memBenefitErrorMagPlot <- ggplot(subIndices, aes(y = memBenefitIndex, x = meanFreqErrorMag)) +
  geom_point(color = color3) +
  geom_smooth(method = "lm", color = color3, fill = color3) +
  xlab("Frequency Response Error Magnitude") +
  ylab("Mem Benefit Index") +
  kate.theme
memBenefitErrorMagPlot
```
...
memFreqPlotData <- memFreqData %>%  
  mutate(IQGroup = ntile(IQ, 2)) %>% 
  group_by(ageGroup, IQGroup, memFreqResp) %>% 
  summarise(meanAcc = mean(memAcc, na.rm = T))
...
memByExplicitFreq1 <- memData[which(memData$memFreqResp >0 & memData$freqCond == 1),] %>%
  group_by(memFreqResp, freqCond, ageGroup) %>% 
  summarise(meanAcc = mean(memAcc), 
            sdAcc= sd(memAcc), N = n(),
            seAcc = sdAcc/(sqrt(N))) 

memByExplicitFreq5 <- memData[which(memData$memFreqResp >0 & memData$freqCond == 5),] %>%
  group_by(memFreqResp, freqCond, ageGroup) %>% 
  summarise(meanAcc = mean(memAcc), 
            sdAcc= sd(memAcc), N = n(),
            seAcc = sdAcc/(sqrt(N))) 
...
## Model comparison: Frequency condition vs. frequency report 
...
## Effects of frequency response error magnitude on memory
```{r item-level freq report error magnitude and memory data processing}
#scale frequency report error
memFreqData$memFreqRespErrorMagScaled <- scale_this(memFreqData$memFreqRespErrorMagnitude)
```

```{r mem freq error mag relation model with IQ interaction, eval = F}
memFreqErrorMagRelation.IQ.maximal <- mixed(memAcc ~ ageScaled * memFreqRespErrorMagScaled * block * IQScaled + (memFreqRespErrorMagScaled * block|sub) + (ageScaled * memFreqRespErrorMagScaled * block * IQScaled|item), data = memFreqData, family = "binomial", method = "LRT", control = glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)))

#does not converge
```

```{r mem freq error relation mag model with IQ interaction 2, eval = F}
memFreqErrorMagRelation.IQ.2 <- mixed(memAcc ~ ageScaled * memFreqRespErrorMagScaled * block * IQScaled + (memFreqRespErrorMagScaled * block||sub) + (ageScaled * memFreqRespErrorMagScaled * block * IQScaled||item), data = memFreqData, family = "binomial", method = "LRT", control = glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)), expand_re = T)

#does not converge
```

```{r mem freq error relation mag model with IQ interaction 3}
#Remove interactions from random slopes
memFreqErrorMagRelation.IQ.3 <- mixed(memAcc ~ ageScaled * memFreqRespErrorMagScaled * block * IQScaled + (memFreqRespErrorMagScaled + block||sub) + (ageScaled + memFreqRespErrorMagScaled + block + IQScaled||item), data = memFreqData, family = "binomial", method = "LRT", control = glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)), expand_re = T)

nice(memFreqErrorMagRelation.IQ.3)

memFreqErrorMagRelation.IQ.3.glmer <- mixed(memAcc ~ ageScaled * memFreqRespErrorMagScaled * block * IQScaled + (memFreqRespErrorMagScaled + block||sub) + (ageScaled + memFreqRespErrorMagScaled + block + IQScaled||item), data = memFreqData, family = "binomial", method = "LRT", control = glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)), expand_re = T, return = "merMod")
```


## Effects of frequency response error magnitude on memory with frequency condition in model
```{r mem freq error relation mag model with freqCond interaction, eval = F}
memFreqErrorMagFreqCondRelation <- mixed(memAcc ~ ageScaled * memFreqRespErrorMagScaled * freqCondFactor * block * IQScaled + (memFreqRespErrorMagScaled + block + freqCondFactor|sub) + (ageScaled * memFreqRespErrorMagScaled * freqCondFactor * block * IQScaled|item), data = memFreqData, family = "binomial", method = "LRT", control = glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)))

#does not converge
```

```{r mem freq error relation mag model with freqCond interaction 2, eval = F}
# Remove correlations in REs
memFreqErrorMagFreqCondRelation.2 <- mixed(memAcc ~ ageScaled * memFreqRespErrorMagScaled * freqCondFactor * block * IQScaled + (memFreqRespErrorMagScaled + block + freqCondFactor||sub) + (ageScaled * memFreqRespErrorMagScaled * freqCondFactor * block * IQScaled||item), data = memFreqData, family = "binomial", method = "LRT", control = glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)), expand_re = T)

#does not converge
```

```{r mem freq error relation mag model with freqCond interaction 3}
# Remove interactionsin REs
memFreqErrorMagFreqCondRelation.3 <- mixed(memAcc ~ ageScaled * memFreqRespErrorMagScaled * freqCondFactor * block * IQScaled + (memFreqRespErrorMagScaled + block + freqCondFactor||sub) + (ageScaled + memFreqRespErrorMagScaled + freqCondFactor + block + IQScaled||item), data = memFreqData, family = "binomial", method = "LRT", control = glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)), expand_re = T)

nice(memFreqErrorMagFreqCondRelation.3)

#return glmer model
memFreqErrorMagFreqCondRelation.3.glmer <- mixed(memAcc ~ ageScaled * memFreqRespErrorMagScaled * freqCondFactor * block * IQScaled + (memFreqRespErrorMagScaled + block + freqCondFactor||sub) + (ageScaled + memFreqRespErrorMagScaled + freqCondFactor + block + IQScaled||item), data = memFreqData, family = "binomial", method = "LRT", control = glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e6)), expand_re = T, return = "merMod")

```

## Plot: Effects of IQ, Block, Freq Rep Error magnitude on memory
```{r iq block freq rep error plot}
freqErrorIntPlot <- plot_model(memFreqErrorMagFreqCondRelation.3.glmer, type = "pred", terms = c("memFreqRespErrorMagScaled","IQScaled", "block")) +
  kate.theme +
  scale_color_manual(values = c(color1, color2, color3), name = "IQ", labels = c("Low", "Medium", "High"))  +
  scale_fill_manual(values = c(color1, color2, color3), name = "IQ", labels = c("Low", "Medium", "High")) +
  xlab("Frequency Report Error Magnitude") +
  ylab("Memory Accuracy") +
  ggtitle("")
freqErrorIntPlot 
```
